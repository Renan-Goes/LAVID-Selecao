...
#ou: saida = saida.replace(to_replace = '(\()(\+){2,}(\))', value = '(+)', regex = True)

saida = saida.replace(to_replace = r' +', value = r' ', regex = True)

saida = saida.replace(to_replace = r'\b-+\b', value = r'', regex = True)

saida = saida.replace(to_replace = r'( )+_', value = r'_', regex = True)

saida = saida.replace(to_replace = r'(1S|2S|3S|1P|2P|3P)(_)([a-zA-Z])', value = r'\1\2 \3', regex = True)

saida = saida.replace(to_replace = r'(.)( )(\(\++)', value = r'\1\3', regex = True) #considerei que poderia haver mais um '+' por não
                                                                                    #saber se dever ser apenas 1 '+' para intensidade

saida = saida.replace(to_replace = r'(NÃO)( )(\w)', value = r'\1_\3', regex = True)

saida = saida.replace(to_replace = r'([a-zA-Z])(_)(FAMOSO|FAMOSA)', value = r'\1&\3', regex = True) #considerei que _FAMOSO/FAMOSA

saida = saida.replace(to_replace = r'(\.)([^0-9])', value = r'\2', regex = True)                                                                                  #deveria vir depois de uma palavra

saida = saida.replace(to_replace = r'( )(\.)([0-9])', value = r' 0\2\3', regex = True)
